#!/usr/bin/env python
"""Post analyzed engagement comments — resumable via progress file."""
import subprocess
import time
import os
import json

PROGRESS = "engagement_progress.json"
...
def post(pid, sub, text):
    try:
        ...
        print(f"[{'OK ' if ok else 'FAIL'}] r/{sub} {pid}: {(r.stdout or r.stderr).strip()[:110]}", flush=True)
        return ok
...
if __name__ == "__main__":
    ...
    for i, (pid, sub, text) in enumerate(ENGAGE):
        ...
        time.sleep(4)
    print(f"\nDONE: {ok} posted, {fail} failed, {len(done)} total done", flush=True)
