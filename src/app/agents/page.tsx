      ...
      <div className="max-w-6xl mx-auto">
        <div className="flex justify-between items-center mb-8">
          <div>
            ...
            <p className="text-foreground-muted">Manage and deploy AI agents</p>
          </div>
        ...
        <div className="space-y-4">
          {[
            ...
            <div
              key={index}
              className="p-6 bg-background-card rounded-lg border border-border-subtle"
            >
                ...
                <div>
                  <div className="flex items-center space-x-3">
                    ...
                    <span className={`text-sm px-3 py-1 rounded-full ${
                      agent.status === 'Active' ? 'bg-primary-subtle text-primary' :
                      agent.status === 'Idle' ? 'bg-primary-subtle text-primary-light' :
                      'bg-red-500/20 text-red-400'
                    }`}>
                      {agent.status}
                    </span>
                  </div>
                  <p className="text-foreground-muted mt-1">{agent.description}</p>
                </div>
              ...
              <div className="flex items-center justify-between text-sm text-foreground-muted">
                ...
                <div className="flex items-center space-x-3">
                  <button className="text-primary hover:text-primary-light">Configure</button>
                  <button className="text-primary hover:text-primary-light">Deploy</button>
                  <button className="text-primary hover:text-primary-light">View Logs</button>
                </div>
              </div>
            </div>
          ))}
