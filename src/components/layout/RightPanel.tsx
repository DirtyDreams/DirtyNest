...
export default function RightPanel() {
  ...
  if (!isRightPanelOpen) {
    return (
      <aside
        className="hidden xl:flex flex-col items-center py-4 px-1.5 overflow-y-auto shrink-0 transition-all duration-300 border-l border-white/10 fixed right-0 top-0 h-full z-30 select-none pb-10 w-[52px] bg-[#08090F]/95 backdrop-blur-2xl"
      >
        ...
        <Tooltip>
          <TooltipTrigger asChild>
            <Button
              ...
              className="w-9 h-9 rounded-xl bg-white/[0.04] border-white/10 hover:border-[#00FF41]/40 text-[#9499B3] hover:text-[#00FF41] mb-4 shadow-[0_0_10px_rgba(0,0,0,0.5)]"
            >
              <ChevronLeft size={16} />
            </Button>
          </TooltipTrigger>
          <TooltipContent side="left" className="bg-[#090B14] border-white/10 text-xs font-mono text-[#F1F3F9]">
            Expand Tactical Deck (Ctrl+\)
          </TooltipContent>
        </Tooltip>
        ...
        <Tooltip>
          <TooltipTrigger asChild>
            <Button
              ...
              className="w-9 h-9 rounded-xl bg-white/5 border-white/10 text-[#00FF41] mb-3"
            >
              <Zap size={16} />
            </Button>
          </TooltipTrigger>
          <TooltipContent side="left" className="bg-[#090B14] border-white/10 text-xs font-mono text-[#F1F3F9]">
            Mode: {deckMode === "context" ? "Context Deck" : "Classic Deck"}
          </TooltipContent>
        </Tooltip>
  ...
  return (
    <aside
      className="hidden xl:flex flex-col p-3.5 overflow-y-auto overscroll-contain shrink-0 transition-all duration-300 border-l border-white/10 fixed right-0 top-0 h-full z-30 box-border w-[340px] bg-[#08090F]/95 backdrop-blur-2xl"
    >
      ...
      <div className="flex items-center justify-between gap-1.5 mb-3 shrink-0">
        ...
        <div className="flex bg-black/60 p-0.5 rounded-xl border border-white/10 text-[10px] font-mono flex-1">
          <button
            ...
            className={cn(
              "flex-1 py-1.5 px-2 rounded-lg font-bold transition-all flex items-center justify-center gap-1.5 cursor-pointer",
              deckMode === "context"
        ...
        <Button
          ...
          className="h-8 w-8 rounded-xl bg-white/[0.03] border-white/10 hover:border-[#00FF41]/40 text-[#9499B3] hover:text-[#00FF41] shrink-0"
        >
          ...
          {tabs.map((tab) => {
            ...
            return (
              <button
                ...
                onClick={() => handleTabClick(tab.id as RightPanelTab)}
                className={cn(
                  "flex-1 flex items-center justify-center gap-1 py-1.5 rounded-lg transition-all cursor-pointer",
                  isActive
        ...
        {deckMode === "context" && (
          <div className="space-y-3 w-full">
            ...
            <div className="p-2 px-3 rounded-xl bg-black/40 border border-white/5 flex items-center justify-between text-[10px]">
              ...
              <Badge variant="outline" className="text-[#00FF41] border-[#00FF41]/30 bg-[#00FF41]/10 font-bold uppercase">
                {activeView}
              </Badge>
            </div>
